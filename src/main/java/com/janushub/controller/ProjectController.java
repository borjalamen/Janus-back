   ...
   public ResponseEntity<?> deleteProject(@PathVariable String id) {
    ...
    public ResponseEntity<?> uploadProjectDocument(
        ...
        try {
            ...
            // Guardar proyecto actualizado (mantener todos los documentos)
    ...
    private void ensureDocuments(Project project) {
        if (project == null || project.getId() == null) return;
 
        Path projectDir = Paths.get(VOLUMEN, project.getId());
        if (!Files.exists(projectDir) || !Files.isDirectory(projectDir)) return;
 
        Map<String, Project.ProjectDocument> byName = new HashMap<>();
        List<Project.ProjectDocument> existingDocs = project.getDocuments();
        if (existingDocs != null) {
            for (Project.ProjectDocument doc : existingDocs) {
                if (doc != null && doc.getNombre() != null) {
                    byName.put(doc.getNombre(), doc);
                }
            }
        }
 
        List<Project.ProjectDocument> docs = existingDocs != null
                ? new java.util.ArrayList<>(existingDocs)
                : new java.util.ArrayList<>();
 
        try {
            Files.list(projectDir).filter(Files::isRegularFile).forEach(path -> {
                String fileName = path.getFileName().toString();
                Project.ProjectDocument doc = byName.get(fileName);
                if (doc == null) {
                    doc = new Project.ProjectDocument();
                    doc.setNombre(fileName);
                    doc.setDescripcion("");
                    doc.setPath(project.getId() + "/" + fileName);
                    docs.add(doc);
                }
                ...
                if (doc.getTipo() == null || doc.getTipo().isEmpty()) {
                    try {
                        ...
                        doc.setTipo(contentType != null ? contentType : "application/octet-stream");
            ...
            // If listing fails, keep existing documents as-is.
        }
